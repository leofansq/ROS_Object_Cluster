...
#include "object_cluster/ClusterArray.h"
...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in) {
  ...
  object_cluster::ClusterArray cluster_array;
  ...
  if(print_fps_)if(++frames>10){std::cerr<<"[object_cluster] fps = "<<float(frames)/(float(clock()-start_time)/CLOCKS_PER_SEC)<<", timestamp = "<<clock()/CLOCKS_PER_SEC<<std::endl;reset = true;}//fps
}
...
int main(int argc, char **argv) {
  ...
  cluster_array_pub_ = private_nh.advertise<object_cluster::ClusterArray>("clusters", 100);
